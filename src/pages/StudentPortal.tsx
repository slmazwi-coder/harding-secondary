...
import { getStudentDocs, type StudentDoc } from '../admin/utils/storage';

export const StudentPortal = () => {
  const navigate = useNavigate();
  const studentNumber = useMemo(() => sessionStorage.getItem('harding_student_portal_student_number') || '', []);
  const [search, setSearch] = useState('');
  const [docs, setDocs] = useState<StudentDoc[]>([]);

  useEffect(() => {
    if (!studentNumber) navigate('/student/login');
    else setDocs(getStudentDocs(studentNumber));
  }, [studentNumber, navigate]);

  const logout = () => {
    sessionStorage.removeItem('harding_student_portal_student_number');
        ...
        <div className="flex flex-wrap gap-3 items-center justify-between mb-6">
          ...
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full bg-white border border-gray-200 rounded-xl pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-school-primary/20"
            />
          </div>
        </div>
